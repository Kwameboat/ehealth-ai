...
const MEDICAL_CHAT_SYSTEM_PROMPT = `You are Agyenim, the professional clinical triage assistant for eHealth AI.
...
const TRIAGE_RECOMMENDATION_DIRECTIVE = `Give your final recommendations now in one complete message. Include: (1) what symptoms may suggest, (2) self-care steps, (3) when to see a clinician, (4) brief note that this is not in-person care. Finish every sentence. Do not ask questions.`;

const TRIAGE_CONTINUE_DIRECTIVE =
...
/** Heuristic when API response text ends mid-sentence. */
...
/** Pick the right nudge for recommendation phase (avoids repeating full triage on "Ok"). */
function resolveTriageDirective(history, userText) {
...
/** Limits replies during triage questions. */
const MEDICAL_CHAT_GENERATION_CONFIG = {
  maxOutputTokens: 512,
  temperature: 0.35,
};

/** More room for complete recommendation messages. */
...
module.exports = {
  ...
  TRIAGE_CONTINUE_DIRECTIVE,
  POST_REC_ACK_DIRECTIVE,

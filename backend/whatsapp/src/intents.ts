...
export function detectIntent(text: string): UserIntent {
  ...
  if (/^(menu|help|features|options)$/.test(t)) return 'menu';
  if (/\bnhis\b|national health insurance|nhis cover|insurance cover/.test(t)) return 'nhis';
  if (
    /\b(diabetes|hypertension|blood pressure|bp|sugar|type 2|fufu|banku|kontomire|waakye|plantain|diet|eat)\b/.test(t)
  ) {
    return 'diet';
  }
...
export const MENU_TEXT = `🏥 *Agyenim Health Menu*

...
const BreathingProblemsScreen = ({ navigation }) => {
  ...
  const analyzeTextWithGemini = async (text) => {
    ...
    try {
      const prompt = `The user is describing breathing problems: "${text}". 
      Duration: ${symptomDuration || 'Not specified'}
      Breathing Pattern: ${breathingPattern || 'Not specified'}
      ...
      Pay special attention to potentially serious conditions like asthma, 
      COPD, pneumonia, or other respiratory emergencies.`;
  ...
  const analyzeImageWithGemini = async (imageUri) => {
    ...
    try {
      ...
      Provide a detailed analysis of potential breathing problems, symptoms, 
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Breathing Problems Analysis</Text>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {/* Symptoms Input */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Describe Your Breathing Issues</Text>
          
          {/* Duration Input */}
          <Text style={styles.inputLabel}>How long have you had these symptoms?</Text>
          <TextInput
            style={styles.input}
            placeholder="e.g., 2 hours, 3 days, 1 week, etc."
          ...
          <Text style={styles.inputLabel}>Describe your breathing pattern</Text>
          <View style={styles.patternContainer}>
            {['Normal', 'Rapid', 'Shallow', 'Labored', 'Wheezing'].map(pattern => (
              <TouchableOpacity
                key={pattern}
                style={[
                  styles.patternButton,
                  breathingPattern === pattern && { backgroundColor: theme.colors.primary }
                ]}
                onPress={() => setBreathingPattern(pattern)}
              >
                <Text style={[
                  styles.patternText,
                  breathingPattern === pattern && { color: theme.colors.buttonText }
                ]}>
                  {pattern}
                </Text>
              </TouchableOpacity>
          ...
          <TextInput
            ...
            placeholder="Describe your breathing difficulties, triggers, when it worsens, etc..."
            placeholderTextColor={theme.colors.textSecondary}
            multiline
            value={userNotes}
        ...
        <View style={[styles.warningContainer, { backgroundColor: theme.colors.danger + '20' }]}>
          ...
          <Text style={[styles.warningText, { color: theme.colors.danger }]}>
            Seek immediate medical attention if experiencing severe shortness of breath, blue lips/fingernails, or chest pain.
          </Text>
        </View>
        ...
        {analysisResult ? (
          ...
          <View style={[styles.placeholderContainer, { backgroundColor: theme.colors.card }]}>
            ...
            <Text style={[styles.placeholderText, { color: theme.colors.text }]}>
              Describe your breathing symptoms or upload an image to get started
            </Text>
          </View>
        )}
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, { backgroundColor: theme.colors.card }]}>
            <Text style={[styles.modalTitle, { color: theme.colors.text }]}>
              Describe Your Breathing Symptoms
            </Text>
...
export default BreathingProblemsScreen;
...
const DentalPainScreen = ({ navigation }) => {
  ...
  const analyzeTextWithGemini = async (text) => {
    ...
    try {
      const prompt = `The user is describing dental pain: "${text}". 
      ...
      Please provide a detailed analysis of potential dental conditions, 
      symptoms, causes, and recommendations for treatment and medication. 
      Also, suggest when to consult a dentist. Format your response with 
      clear sections for Analysis, Recommendations, and When to Seek Help.
      Pay special attention to potentially serious conditions like abscesses, 
      infections, or other dental emergencies.`;
  ...
  const analyzeImageWithGemini = async (imageUri) => {
    ...
    try {
      ...
      const prompt = `Analyze this image of a dental condition. 
      Provide a detailed analysis of potential dental problems, symptoms, 
      causes, and recommendations for treatment and medication. Also, 
      suggest when to consult a dentist. Format your response with clear 
  ...
  const renderAnalysisResult = () => {
    ...
    return sections.map((section, index) => {
      ...
      if (section.includes('When to Seek Help:')) {
        return (
          <View key={index} style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>When to Consult a Dentist</Text>
            <Text style={styles.resultText}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Dental Pain Analysis</Text>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {/* Symptoms Input */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Describe Your Dental Pain</Text>
          ...
          <View style={styles.patternContainer}>
            {['Throbbing', 'Sharp', 'Dull', 'Constant', 'Intermittent'].map(type => (
              <TouchableOpacity
                key={type}
                style={[
          ...
          {/* Pain Location Input */}
          <Text style={styles.inputLabel}>Where is the pain located?</Text>
          <View style={styles.patternContainer}>
            {['Upper left', 'Upper right', 'Lower left', 'Lower right', 'Front teeth', 'Back teeth', 'Jaw'].map(location => (
              <TouchableOpacity
                key={location}
                style={[
                  styles.patternButton,
                  painLocation === location && { backgroundColor: theme.colors.primary }
                ]}
                onPress={() => setPainLocation(location)}
              >
                <Text style={[
                  styles.patternText,
                  painLocation === location && { color: theme.colors.buttonText }
                ]}>
                  {location}
                </Text>
              </TouchableOpacity>
          ...
          <TextInput
            ...
            placeholder="Describe your dental pain, triggers, when it worsens, etc..."
            placeholderTextColor={theme.colors.textSecondary}
            multiline
            value={userNotes}
        ...
        <View style={[styles.warningContainer, { backgroundColor: theme.colors.danger + '20' }]}>
          ...
          <Text style={[styles.warningText, { color: theme.colors.danger }]}>
            Seek immediate dental attention if experiencing severe pain, facial swelling, fever, or difficulty breathing.
          </Text>
        </View>
        ...
        {analysisResult ? (
          ...
          <View style={[styles.placeholderContainer, { backgroundColor: theme.colors.card }]}>
            ...
            <Text style={[styles.placeholderText, { color: theme.colors.text }]}>
              Describe your dental symptoms or upload an image to get started
            </Text>
          </View>
        )}
      </ScrollView>
...
export default DentalPainScreen;
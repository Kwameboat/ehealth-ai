...
export async function fetchWithRetry(url, options = {}, attempts = 3, timeoutMs = 55_000) {
  let lastErr;
  const isSlowRoute = String(url).includes('/api/chat') || String(url).includes('/api/health/nhis') || String(url).includes('/api/health/diet');
  const maxAttempts = isSlowRoute ? Math.min(attempts, 2) : attempts;
  const waitMs = isSlowRoute ? 600 : 900;
